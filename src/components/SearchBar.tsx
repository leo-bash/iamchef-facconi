...
import { ingredientsMock, recipesMock } from "../mock/mock";
...
function SearchBar({ onViewChange }: SearchBarProps) {
  ...
  const [filteredResults, setFilteredResults] = useState<IngredientInterface[]>([]);
  ...
  const handleSearchRecipes = () => {
    ...
    const filtered = recipesMock.filter((recipe: any) => {
  ...
  const handleChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (value.trim() === "") {
      ...
      const filtered = ingredientsMock.filter((ingredient: IngredientInterface) =>
  ...
  const handleAddIngredient = (ingredient: IngredientInterface) => {
    ...
    return (
      <div className="recipeDetail">
        <div className="detailHeader">
          <button className="backButton" onClick={handleBackToResults}>
        ...
        <img src={selectedRecipe.image} alt={selectedRecipe.title} className="recipeImage" />
    ...
    <section className="heroSection">
      ...
      {selectedIngredients.length > 0 && (
        <div className="selectedIngredients">
          <span className="selectedLabel">Selected ingredients</span>
          <SelectedList ingredients={selectedIngredients} handleRemove={(ingredient) => handleRemoveIngredient(ingredient.id)} currentPage={{ currentPage: { page: "homepage" } }} />
          <button
            className="searchBtn"
            type="button"
      ...
      {filteredResults.length > 0 && !showResults && (
        <div className="resultsBox">
          {filteredResults.map((ingredient) => (
            <div key={ingredient.id} className="resultRow">
              <button
                type="button"
                className="resultBtn"
          ...
          {foundRecipes.length === 0 ? (
            ...
            foundRecipes.map(recipe => (
              <div key={recipe.id} onClick={() => setSelectedRecipe(recipe)} style={{ cursor: 'pointer' }}>
                <RecipeCard recipe={recipe} onClickDetails={() => setSelectedRecipe(recipe)} />
              </div>
            ))
          )}
          <button
